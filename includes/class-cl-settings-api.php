...
class CL_Settings_API {
	...
	public function init() {
		...
		add_action( $this->settings['prefix'] . '_settings_sidebars',		array( $this, 'sidebar_feed' ), 19 );
	}
	...
	function about_the_author( $args ) {
		...
		$content  = '<ul>';
		$content .= '<li>Author: <a href="http://austin.passy.co">Austin Passy</a></li>';
		$content .= '</ul>';
		
		$this->postbox( 'frosty-media-author', __( 'About', $this->settings['domain'] ), $content );
	}
	...
	function sidebar_feed( $args ) {
		...
		$defaults = array(
			'items' => 6,
			'feed' 	=> 'https://extendd.com/feed/?post_type=download&download_tag=custom-login-extension',
		);
	...
	public function sticky_admin_notice_social_links() {
		...
		$content .= '<li><a href="https://plus.google.com/+FrostyMediaWP/"><span class="dashicons dashicons-googleplus"></span></a></li>';
