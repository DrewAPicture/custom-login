...
class CL_Tracking {
	...
	public function __construct() {
		...
		$this->api 	= defined( 'WP_LOCAL_DEV' ) ? CUSTOM_LOGIN_API_URL . 'cl-checkin-api?edd_action=cl_checkin' : CUSTOM_LOGIN_API_URL . 'cl-checkin-api?edd_action=cl_checkin';
		$this->schedule_send();
		
		add_action( CUSTOM_LOGIN_OPTION . '_after_sanatize_options',	array( $this, 'check_for_settings_optin' ) );
		add_action( 'admin_action_opt_into_tracking',					array( $this, 'check_for_optin' ) );
		add_action( 'admin_action_opt_out_of_tracking',					array( $this, 'check_for_optout' ) );
	...
	private function tracking_allowed() {
		return CL_Common::get_option( 'tracking', 'general', false );
	}
	...
	public function check_for_optout( $data ) {
		...
		if ( isset( $options['tracking'] ) ) {
			unset( $options['tracking'] );
			update_option( $this->option, $options );
		}

		update_option( 'cl_tracking_notice', '1' );

		wp_redirect( remove_query_arg( 'action' ) ); exit;
	...
	public function admin_notice() {
		...
		if ( 
			...
			$optin_url  = add_query_arg( 'action', 'opt_into_tracking' );
			$optout_url = add_query_arg( 'action', 'opt_out_of_tracking' );

			echo '<div class="updated"><p>';
				echo __( 'Allow Frosty Media to anonymously track how this plugin is used and help us make the plugin better. Opt-in and receive a 20% discount code for any purchase from <a href="https://frosty.media/plugins" target="_blank">Frosty Media plugins</a>. No sensitive data is tracked.', CUSTOM_LOGIN_DIRNAME );
