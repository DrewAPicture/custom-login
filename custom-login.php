 ...
 * Description: A simple way to customize your WordPress <code>wp-login.php</code> screen! Visit the <a href="./options-general.php?page=custom-login">settings</a> page and customize away. Share you designs on <a href="http://flickr.com/groups/custom-login/">Flickr</a>, and browse all the Custom Login extensions at <a href="https://frosty.media/plugins/tag/custom-login-extension">Extendd.com</a>.
 * Version: 3.0.0
 * Author: Austin Passy
...
final class Custom_Login {
	...
	private function setup_constants() {
		...
		if ( ! defined( 'CUSTOM_LOGIN_API_URL' ) ) {
			define( 'CUSTOM_LOGIN_API_URL', defined( 'WP_LOCAL_DEV' ) ? 'http://frosty.media.dev/' : 'https://frosty.media/' );
		}
	...
	*/
	...
	public function admin_menu() {
		...
		add_action( 'load-' . $this->menu_page,				array( $this, 'remote_install_client' ), 9 );
	}
	...
	public function remote_install_client() {
		...
		$cl_remote_install_client = new CL_Remote_Install_Client(
			...
			$this->menu_page,
			array( 'skipplugincheck' => false, 'url' => home_url() )
		);
	}
	...
	public function load_extensions() {
		...
		$cl_extensions = new CL_Extensions();		
		ob_start();
		$cl_extensions->html();
		return ob_get_clean();
	}
    ...
    function show_notifications() {
		...
		if ( !$is_cl_screen ) {
			...
			if ( 'off' === CL_Common::get_option( 'admin_notices', 'general', 'off' ) ) return;
